(() => {
  ...
  kintone.events.on(['mobile.app.record.create.show', 'mobile.app.record.edit.show'], (event) => {
    ...
    kintone.mobile.app.record.getSpaceElement(barcodeWrapper).appendChild(div);
    ...
    // バーコード起動停止ボタン
    // eslint-disable-next-line no-undef
    const cameraButton = new Kuc.MobileButton({
      id: barcodeButton,
      text: barcodeButtonText.init,
      type: 'submit',
      className: 'ui-component-edit-margin',
    });
    kintone.mobile.app.record.getSpaceElement(barcodeButton).appendChild(cameraButton);

    // 本情報取得ボタン
    // eslint-disable-next-line no-undef
    const getBookDataButton = new Kuc.MobileButton({
      id: isbnButton,
      text: '本の情報を取得',
      type: 'submit',
      className: 'ui-component-edit-margin',
    });
    kintone.mobile.app.record.getSpaceElement(isbnButton).appendChild(getBookDataButton);
    // サムネイル表示領域
    ...
    kintone.mobile.app.record.getSpaceElement(thumbnailWrapper).appendChild(img);
  ...
  kintone.events.on(['mobile.app.record.detail.show'], (event) => {
    ...
    kintone.mobile.app.record.getSpaceElement(thumbnailWrapper).appendChild(img);
    // スペースフィールドを非表示
    let space = kintone.mobile.app.record.getSpaceElement(barcodeWrapper);
    space.parentNode.style.display = 'none';
    space = kintone.mobile.app.record.getSpaceElement(barcodeButton);
    space.parentNode.style.display = 'none';
    space = kintone.mobile.app.record.getSpaceElement(isbnButton);
    space.parentNode.style.display = 'none';
  });
})();
